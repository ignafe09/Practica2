...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar 14 17:42:27 2023
...
En esta opcion se cede el turno al que tenga mas esperando para entrar, dandole prioridad.
Ademas solo pueden entrar si es su turno o si no hay ninguno de las otras clases esperando.
Puede producir inanición si la cantidad de un tipo que vienen por un lado es muy superior a
la de los demás, tal y como está programado no sucede. En el modelo anterior esto no se produce.
"""
import time
import random
...
class Monitor():
    ...
    def leaves_car(self, direction: int) -> None:
        ...
        if(direction==SOUTH):
            self.Scoches.value-=1
            if (self.Nwaiting.value>=self.Swaiting.value and self.Nwaiting.value>=self.Pwaiting.value):
                self.turn.value = 1
            elif(self.Pwaiting.value>=self.Swaiting.value and self.Pwaiting.value>=self.Nwaiting.value):
                self.turn.value=0
            self.puede_pasar_north.notify_all()
            self.puede_pasar_peaton.notify_all()  
        if(direction==NORTH):
            self.Ncoches.value-=1
            if(self.Pwaiting.value>=self.Swaiting.value and self.Pwaiting.value>=self.Nwaiting.value):
                self.turn.value=0
            elif(self.Swaiting.value>=self.Pwaiting.value and self.Swaiting.value>=self.Nwaiting.value):
                self.turn.value=2
            self.puede_pasar_peaton.notify_all() 
    ...
    def leaves_pedestrian(self) -> None:
        ...
        self.patata.value += 1
        self.np.value-=1
        if(self.Swaiting.value>=self.Nwaiting.value and self.Swaiting.value>=self.Pwaiting.value):
            self.turn.value = 2
        elif(self.Nwaiting.value>=self.Swaiting.value and self.Nwaiting.value>=self.Pwaiting.value):
            self.turn.value=1
        self.puede_pasar_south.notify_all()
        self.puede_pasar_north.notify_all()
        self.mutex.release()
